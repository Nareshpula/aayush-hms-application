...
interface InjectionInvoicePreviewProps {
  invoiceData: {
    invoiceNo: string;
    date: string;
    time: string;
...
const InjectionInvoicePreview: React.FC<InjectionInvoicePreviewProps> = ({ invoiceData, onClose }) => {
  ...
  const handleSaveInvoice = async () => {
    ...
    try {
      const existingInvoice = await DatabaseService.checkInjectionInvoiceExists(invoiceData.invoiceNo);

      if (existingInvoice) {
        setSaveError('Invoice already saved');
        setIsSaving(false);
        return;
      }

      await DatabaseService.saveInjectionInvoice({
        invoice_no: invoiceData.invoiceNo,
        injection_id: invoiceData.injection_id || null,
        patient_id: invoiceData.patient_id,
        doctor_id: invoiceData.doctor_id,
        injection_details: invoiceData.injectionDetails === 'N/A' ? null : invoiceData.injectionDetails,
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[95vh] flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {/* Injection Details Table */}
          <div className="mb-3">
            <div className="bg-gray-200 border border-gray-800 px-2 py-1">
              <h3 className="text-xs font-bold text-gray-900">INJECTION DETAILS</h3>
            </div>
            <table className="w-full border-l border-r border-b border-gray-800" style={{ borderCollapse: 'collapse' }}>
              <tbody>
                <tr>
                  ...
                  <td className="border border-gray-400 px-2 py-1 text-xs" style={{ width: '65%' }}>
                    {invoiceData.injectionDetails || 'N/A'}
                  </td>
                </tr>
              </tbody>
            </table>
...
export default InjectionInvoicePreview;
