/*
  # Create Complete Registration RPC Function

  1. New Functions
    - `create_complete_registration` - One-step patient registration function
      - Creates patient record with auto-generated patient ID
      - Creates registration record with payment tracking
      - Creates IP or OP admission record based on type
      - Returns complete patient and registration data in a single transaction

  2. Function Parameters
    - Patient data: name, contact, age, gender, blood group, address, etc.
    - Registration data: doctor_id, registration_type, appointment_date
    - Payment data: payment_method (Cash/UPI), payment_amount
    - IP-specific: guardian info, admission details, room number
    - OP-specific: appointment time, consultation type, symptoms

  3. Return Value
    - JSON object with success status, patient data, registration data, and generated patient ID
*/

-- Drop function if it exists to ensure clean creation
DROP FUNCTION IF EXISTS public.create_complete_registration CASCADE;

-- Create the complete registration function
